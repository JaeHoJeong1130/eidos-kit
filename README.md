...
제품 코드와 도메인 규칙은 각 프로젝트가 소유합니다.

2026-09-16의 제한된 실제 모델 A/B 실험에서, 진입 지침 경량화 후 구조 설명 과제의 평균 입력 토큰은 **48.6%**,
입력+출력 토큰은 **48.5%**, 경과 시간은 **47.7%** 감소했습니다. 개선 전·후 각 2회가 자동 정답·보존 검사를 통과한 결과입니다.
이는 키트 사용·미사용 비교가 아니며, 합성 기록 조회 과제는 개선판의 시간 초과로 비교를 완료하지 못했습니다.
다양한 실무에서의 품질·생산성 및 일반적인 토큰 절감 효과는 추가 검증이 필요합니다.
조건·시행별 수치·한계는 [실험 결과 요약](_evidence/eidos-ab-native-20260916.md)을 참고하세요.
...
이 초기 배포는 공용 키트, 가상 데이터를 사용하는 테스트와 제한된 모델 실험의 결과 요약을 포함합니다. EidosWeb의 팀 기록, 설치 registry,
